...
@Module class DataSources {
    ...
    @AppUserRepo
    @ApplicationScope
    internal fun provideUserRepository(firebaseAuth: FirebaseAuth, @LocalUserSource local: UserSource, @RemoteUserSource remote: UserSource) : IUserRepo =
            UserRepoImpl(firebaseAuth = firebaseAuth, local = local, remote = remote, schedulerProvider = ThreadProvider, localData = LocalDataSourceImpl())
}