...
class ListPresenterImpl(val list: ListModel) : ListContract.ListPresenter {
    ...
    override fun findWhoIsShopping(numberOfUser: Int) {
        when(numberOfUser) {
            0 -> view.displayNoOneIsShopping()
            1 -> view.displayOnePersonIsShopping()
            else -> view.displayManyPeopleAreShopping(numberOfUser)
        }
    }

    override fun loadList() {
        view.displayList(list)
        view.displayUsersShopping(this)
    }
