 ...
 */
class ItemPresenterImpl(val item: ItemModel, val currentUserName: String, private val updateEvent: PublishSubject<ItemModel>) : ItemContract.ItemPresenter{
    ...
    override fun onItemChecked(checked: Boolean) {
        item.brought = checked

        /******* Set the name of who brought the item ****/
        if (checked) view.displayItemBroughtBy(currentUserName)
        else view.hideItemBroughtBy()
